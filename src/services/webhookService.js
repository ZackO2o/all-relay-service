...
class WebhookService {
  ...
  async sendToBark(platform, type, data) {
    const payload = {
      ...
      group: platform.group || 'all-relay',
      badge: 1
    }
  ...
  async sendToSMTP(platform, type, data) {
    try {
      ...
      const mailOptions = {
        ...
        subject: `[ALL Relay Service] ${subject}`,
        text: textContent,
        html: htmlContent
      }
  ...
  async sendHttpRequest(url, payload, timeout, axiosOptions = {}) {
    const headers = {
      ...
      'User-Agent': 'all-relay-service/2.0',
  ...
  formatMessageForTelegram(type, data) {
    ...
    const lines = [`${title}`, '服务: ALL Relay Service']
  ...
  formatMessageForEmail(type, data) {
    ...
    let content = `
      <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif;">
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; border-radius: 8px 8px 0 0;">
          ...
          <p style="margin: 10px 0 0 0; opacity: 0.9;">ALL Relay Service</p>
        </div>
    ...
    content += `
          ...
          <div style="margin-top: 20px; padding-top: 16px; border-top: 1px solid #e9ecef; font-size: 14px; color: #6c757d; text-align: center;">
            <p>发送时间: ${timestamp}</p>
            <p style="margin: 0;">此邮件由 ALL Relay Service 自动发送</p>
          </div>
        </div>
      </div>
    `
  ...
  async testWebhook(platform) {
    try {
      const testData = {
        message: 'ALL Relay Service webhook测试',
        timestamp: getISOStringWithTimezone(new Date())
      }
